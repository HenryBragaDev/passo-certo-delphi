object TelaRemoverCliente: TTelaRemoverCliente
  Left = 0
  Top = 0
  Align = alClient
  BorderStyle = bsNone
  Caption = 'TelaRemoverCliente'
  ClientHeight = 880
  ClientWidth = 1139
  Color = clBackground
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = 'Segoe UI'
  Font.Style = []
  Position = poDesktopCenter
  OnActivate = FormActivate
  OnShow = FormShow
  TextHeight = 15
  object pnlFundo: TPanel
    Left = 39
    Top = 8
    Width = 1042
    Height = 769
    BevelOuter = bvNone
    Color = clBackground
    ParentBackground = False
    TabOrder = 0
    object pnlborda: TPanel
      Left = 128
      Top = 88
      Width = 801
      Height = 569
      BevelOuter = bvNone
      Color = 13487565
      ParentBackground = False
      TabOrder = 0
      DesignSize = (
        801
        569)
      object shpCriarProduto: TShape
        Left = 0
        Top = 55
        Width = 801
        Height = 562
        Brush.Color = clBtnFace
      end
      object speedbtn_fechar: TSpeedButton
        Left = 776
        Top = 10
        Width = 17
        Height = 17
        Cursor = crHandPoint
        Anchors = [akTop, akRight]
        BiDiMode = bdLeftToRight
        Caption = 'X'
        Flat = True
        Font.Charset = ANSI_CHARSET
        Font.Color = 4802889
        Font.Height = -33
        Font.Name = 'Arrial Narrow'
        Font.Style = []
        ParentFont = False
        ParentBiDiMode = False
        OnClick = speedbtn_fecharClick
      end
      object lblTitulo: TLabel
        Left = 24
        Top = 15
        Width = 155
        Height = 24
        Caption = 'Remover Cliente'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -20
        Font.Name = 'Poppins'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object pnlItens: TPanel
        Left = 0
        Top = 48
        Width = 801
        Height = 566
        BevelOuter = bvNone
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -19
        Font.Name = 'Segoe UI'
        Font.Style = []
        ParentBackground = False
        ParentFont = False
        TabOrder = 0
        object lblNome: TLabel
          Left = 59
          Top = 103
          Width = 71
          Height = 29
          Caption = 'Email:'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -24
          Font.Name = 'Poppins'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object lbltelefone: TLabel
          Left = 59
          Top = 171
          Width = 104
          Height = 29
          Caption = 'Telefone:'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -24
          Font.Name = 'Poppins'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object lblPreco: TLabel
          Left = 59
          Top = 242
          Width = 203
          Height = 29
          Caption = 'Data de Cadastro:'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -24
          Font.Name = 'Poppins'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object shpSalvar: TShape
          Left = 387
          Top = 418
          Width = 145
          Height = 41
          Brush.Color = 7091712
          Shape = stRoundRect
        end
        object shpCancelar: TShape
          Left = 567
          Top = 418
          Width = 138
          Height = 41
          Shape = stRoundRect
        end
        object SpeedbtnSalvar: TSpeedButton
          Left = 388
          Top = 419
          Width = 142
          Height = 39
          Caption = 'Salvar'
          Flat = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -20
          Font.Name = 'Poppins'
          Font.Style = [fsBold]
          ParentFont = False
          OnClick = SpeedbtnSalvarClick
        end
        object SpeedbtnCancelar: TSpeedButton
          Left = 569
          Top = 419
          Width = 135
          Height = 39
          Caption = 'Cancelar'
          Flat = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -20
          Font.Name = 'Poppins'
          Font.Style = [fsBold]
          ParentFont = False
          OnClick = SpeedbtnCancelarClick
        end
        object shpCor: TShape
          Left = 0
          Top = 0
          Width = 801
          Height = 1
          Align = alTop
          Brush.Color = 15724527
          ExplicitWidth = 799
        end
        object lblCategoria: TLabel
          Left = 59
          Top = 39
          Width = 87
          Height = 29
          Caption = 'Cliente:'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -24
          Font.Name = 'Poppins'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object cbCliente: TDBLookupComboBox
          Left = 288
          Top = 38
          Width = 417
          Height = 30
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -19
          Font.Name = 'Poppins'
          Font.Style = []
          ParentFont = False
          TabOrder = 0
          OnCloseUp = cbClienteCloseUp
        end
        object cbEmail: TDBLookupComboBox
          Left = 288
          Top = 103
          Width = 417
          Height = 30
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -19
          Font.Name = 'Poppins'
          Font.Style = []
          ParentFont = False
          TabOrder = 1
          OnCloseUp = cbEmailCloseUp
        end
        object cbTelefone: TDBLookupComboBox
          Left = 288
          Top = 171
          Width = 225
          Height = 30
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -19
          Font.Name = 'Poppins'
          Font.Style = []
          ParentFont = False
          TabOrder = 2
          OnCloseUp = cbTelefoneCloseUp
        end
        object cbDataCadastro: TDBLookupComboBox
          Left = 288
          Top = 241
          Width = 225
          Height = 30
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -19
          Font.Name = 'Poppins'
          Font.Style = []
          ParentFont = False
          TabOrder = 3
        end
      end
    end
  end
  object ADOQueryClienteNome: TADOQuery
    Connection = DMPrincipal.ADOConnectionGeral
    Parameters = <>
    Left = 47
    Top = 704
  end
  object DataSourceClienteNome: TDataSource
    DataSet = ADOQueryClienteNome
    Left = 111
    Top = 688
  end
  object ADOQueryClienteEmail: TADOQuery
    Connection = DMPrincipal.ADOConnectionGeral
    Parameters = <>
    Left = 383
    Top = 696
  end
  object DataSourceClienteEmail: TDataSource
    DataSet = ADOQueryClienteEmail
    Left = 423
    Top = 688
  end
  object ADOQueryClienteTelefone: TADOQuery
    Connection = DMPrincipal.ADOConnectionGeral
    Parameters = <>
    Left = 599
    Top = 704
  end
  object DataSourceClienteTelefone: TDataSource
    DataSet = ADOQueryClienteTelefone
    Left = 647
    Top = 680
  end
  object ADOQueryClienteData: TADOQuery
    Connection = DMPrincipal.ADOConnectionGeral
    Parameters = <>
    Left = 815
    Top = 704
  end
  object DataSourceClienteData: TDataSource
    DataSet = ADOQueryClienteData
    Left = 871
    Top = 688
  end
end
